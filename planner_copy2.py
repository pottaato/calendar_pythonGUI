...
<input type=submit value="yearly" align=left onclick="yearly({},{})">
<input type=button value="weekly" align=left onclick="weekly({},{},{});">
""".format(year, month, action, name_month, year, year, month, year, month, day)
        ...
        if m == int(month):
            ...
            for i in cur.fetchall():
                ...
                for j in db_cont:
                    ls.append(j)
                if len(db_cont) > 7:
                    con = ls[0]+ls[1]+ls[2]+ls[3]+ls[4]+ls[5]+"..."
                else:
                    con = i[6]
                print("<br><div onclick='show_edit({});'>{}:{}|{}</div>".format(i[0],i[4],i[5],con))
            print("</td>")
        else:
            print("<td ondblclick='show_add({},{},{});'".format(y,m,d))
            if y == int(year) and m == int(month) and d == int(day) and action is not None:
...
print("""
        ...
        <script language='javascript'>
            function show_edit(eid) {location.href='planner_copy2.py?action=show_edit&eid='+eid;}
            function show_add(year, month, date) 
                {location.href='planner_copy2.py?action=show_add&year='+year+'&month='+month+'&day='+date;}
            function back() {location.href = 'planner_copy2.py';}
            function weekly(year, month, day) {location.href = '/cgi-bin/planner_weekly1.py?year='+year+'&month='+month+'&day='+day;}
            function yearly(year, month) {
                location.href='/cgi-bin/planner_yearly1.py?year='+year+'&month='+month;}
        </script>
        <body>
        <form action='/cgi-bin/planner_copy2.py' method='GET'>
        """)
